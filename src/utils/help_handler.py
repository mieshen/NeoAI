    ...
    while True:
        ...
        if user_input.upper().startswith(".BASE_URL"):
            ...
            output_handler.print_to_console(
                "help_base_url_updated", url=config["API_BASE_URL"]
            )
            ...
            if level.isdigit() and 0 <= int(level) <= 3:
                ...
                output_handler.print_to_console(
                    "help_level_updated", level=config["EXECUTION_LEVEL"]
                )
            else:
            ...
            if timeout.isdigit() or timeout == "-1":
                ...
                output_handler.print_to_console(
                    "help_timeout_updated", timeout=config["RETURN_TIMEOUT"]
                )
            else:
            ...
            try:
                ...
                if 0 <= temp_value <= 1:
                    ...
                    output_handler.print_to_console(
                        "help_temperature_updated", temperature=config["TEMPERATURE"]
                    )
                else:
            ...
            if max_tokens.isdigit():
                ...
                output_handler.print_to_console(
                    "help_max_tokens_updated", max_tokens=config["MAX_TOKENS"]
                )
            else:
            ...
            if max_turns.isdigit():
                ...
                output_handler.print_to_console(
                    "help_max_turns_updated", max_turns=config["MAX_TURNS"]
                )
            else:
            ...
            output_handler.print_to_console(
                "help_show_base_url", url=config["API_BASE_URL"]
            )
            ...
            output_handler.print_to_console(
                "help_show_level", level=config["EXECUTION_LEVEL"]
            )
        elif user_input.upper() == ".SHOW_TIMEOUT":
            output_handler.print_to_console(
                "help_show_timeout", timeout=config["RETURN_TIMEOUT"]
            )
        elif user_input.upper() == ".SHOW_TEMPERATURE":
            output_handler.print_to_console(
                "help_show_temperature", temperature=config["TEMPERATURE"]
            )
        elif user_input.upper() == ".SHOW_MAX_TOKENS":
            output_handler.print_to_console(
                "help_show_max_tokens", max_tokens=config["MAX_TOKENS"]
            )
        elif user_input.upper() == ".SHOW_MAX_TURNS":
            output_handler.print_to_console(
                "help_show_max_turns", max_turns=config["MAX_TURNS"]
            )
            ...
            try:
                selection = int(
                    input(output_handler.get_translation("select_language")).strip()
                )
                if 1 <= selection <= len(available_languages):
                    ...
                    output_handler.print_to_console(
                        "help_language_updated",
                        language=available_languages[selection - 1][1],
                    )
